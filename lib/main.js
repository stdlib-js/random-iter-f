...
function iterator( d1, d2, options ) {
	...
	if ( !isPositiveNumber( d1 ) ) {
		throw new TypeError( format( '0Pj7P', d1 ) );
	}
	if ( !isPositiveNumber( d2 ) ) {
		throw new TypeError( format( '0Pj7T', d2 ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0Pj2h', options ) );
		}
		opts = copy( options, 1 );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0Pj35', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
